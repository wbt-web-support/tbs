...
export function FloatingChat() {
  ...
  // Function to refresh chat instances (useful for auto-title updates)
  const refreshCurrentInstance = async () => {
    if (currentInstanceId) {
      try {
        const { data: { session } } = await supabase.auth.getSession();
        if (!session?.user) return;

        const response = await fetch(`/api/gemini?action=instance&instanceId=${currentInstanceId}`, {
          headers: {
            'Authorization': `Bearer ${session.access_token}`
          }
        });
            ...
            setChatInstances(prev => 
              prev.map(instance => 
                instance.id === currentInstanceId 
                  ? { ...instance, title: data.instance.title, updated_at: data.instance.updated_at }
                  : instance
              )
            );
          }
        ...
        console.log('Failed to refresh instance:', error);
      }
    }
  };
            ...
            <div className="h-full flex flex-col">
              ...
              <div className="flex-1 overflow-y-auto">
                <RealtimeChatGemini 
                  ...
                  onTitleUpdate={refreshCurrentInstance}
                />
              </div>
            </div>
          </div>
