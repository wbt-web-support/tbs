  ...
  "questions": [
    {
      ...
      "question_category": "Strategic Planning|Mission & Vision|Core Values|Strategic Anchors|Purpose/Why|Targets & Goals|Business Planning|Operations",
...
// Default fallback prompt template if database entry is missing
const DEFAULT_BUSINESS_PLAN_QUESTIONS_PROMPT = `You are an expert business consultant helping to create a comprehensive business plan.

A business plan should include:
1. Mission Statement: The company's core purpose and reason for existence
2. Vision Statement: The company's long-term aspirations and desired future state
3. Core Values: The fundamental beliefs and principles that guide the company
4. Strategic Anchors: Key strategic priorities that anchor decision-making
5. Purpose/Why: The deeper reason the company exists beyond profit
6. Targets & Goals: Short-term (1 year), medium-term (3 years), and long-term (10 years) objectives

Based on this comprehensive business information, generate 6-10 personalized questions to help create a tailored business plan.

{{businessContext}}{{machinesContext}}

CRITICAL: Generate questions that specifically help determine:
1. What the company's core mission and purpose are
2. What the company's long-term vision and aspirations are
3. What core values guide the company's culture and decisions
4. What strategic anchors should guide all business decisions
5. What the deeper purpose/why behind the business is
6. What specific targets and goals should be set for 1, 3, and 10 years
7. How the business plan should align with existing operations and machines
8. What key challenges and opportunities should be addressed
...
- Questions should directly help create a comprehensive business plan
- Create a MIX of question types:
  * Use "select" for questions with clear, limited options (e.g., choosing between priorities, timeframes, or categories)
  * Use "text" for short, specific answers (e.g., naming a value, identifying a target)
  * Use "textarea" for questions requiring more detailed explanations (e.g., describing mission, vision, or strategic priorities)
- Aim for approximately: 2-3 select questions, 2-3 text questions, 2-4 textarea questions
- Make questions conversational and engaging
- Avoid generic questions - make them specific to their business context
- Ensure questions cover all key business plan components: Mission, Vision, Core Values, Strategic Anchors, Purpose/Why, and Targets & Goals

Examples of good questions:
- Select: "What is your primary strategic focus for the next 12 months?" (options: Revenue Growth, Market Expansion, Operational Efficiency, Product Development, Customer Retention)
- Text: "What is the single most important core value that defines your company culture?"
- Textarea: "Describe your company's mission - what is your core purpose and reason for existence?"
...
- Actions/Activities: ${machine.actionsactivities ? JSON.stringify(machine.actionsactivities) : 'None'}
    ...
    // Load prompt body (instructions) from DB, with fallback to default
    let promptBody = await getPromptBody('business_plan_questions');
    if (!promptBody) {
      console.warn('Prompt body not found in database for business_plan_questions, using default template');
      promptBody = DEFAULT_BUSINESS_PLAN_QUESTIONS_PROMPT;
    }
    ...
    return NextResponse.json({
      success: true,
      message: `Generated ${questionsData.questions.length} personalized questions for business plan creation`,
      questions: questionsWithIds,
      questionsData: questionsDataToStore
    });

  } catch (error) {
    console.error('Error generating business plan questions:', error);
    return NextResponse.json(
      { 
