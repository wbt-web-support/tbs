 ...
 *
 * Manages creating, updating, and deleting voice agents in ElevenLabs.
 */
 ...
 */
function buildToolsConfig(tools: ToolConfig[]): object[] {
...
export async function updateAgent(
  ...
  if (!response.ok) {
    ...
    const detail = typeof error.detail === 'object'
      ...
      : error.detail;
    throw new Error(
      `Failed to update ElevenLabs agent: ${detail || error.message || response.statusText}`
    );
  }
}
