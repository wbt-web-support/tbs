-- Fix all bigint to integer conversion issues in the calculate_remaining_leave_days function
...
BEGIN
  ...
  used_leave AS (
    SELECT COALESCE(SUM(duration_days)::integer, 0) as used_days
  ...
  SELECT 
    COALESCE(e.total_entitlement_days, 25)::integer as total_entitlement,
    COALESCE(ul.used_days, 0)::integer as used_leave_days,
    COALESCE(bh.bank_holiday_count, 0)::integer as bank_holidays,
    (COALESCE(e.total_entitlement_days, 25) - COALESCE(ul.used_days, 0))::integer as remaining_days
  FROM entitlement e
